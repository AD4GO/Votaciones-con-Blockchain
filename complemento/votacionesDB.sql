-- Crea la base de datos "Votaciones" si no existe y la selecciona para su uso
DROP DATABASE IF EXISTS Votaciones;
CREATE DATABASE Votaciones;
USE Votaciones;

-- Crea la tabla "Eleccion" para almacenar información sobre las elecciones
...
-- Crea la tabla "Usuario" para almacenar información sobre los usuarios
...
-- Crea la tabla "Convocatoria" para almacenar información sobre las convocatorias
...
-- Crea la tabla "Administrador" para almacenar información sobre los administradores
...
-- Crea la tabla "Estudiante" para almacenar información sobre los estudiantes
...
-- Crea la tabla "Candidato" para almacenar información sobre los candidatos
...
-- Crea la tabla "Notificacion" para almacenar información sobre las notificaciones
...
-- Crea la tabla "Voto" para almacenar información sobre los votos
CREATE TABLE Voto (
...
-- Define un procedimiento almacenado para obtener candidatos por elección
DELIMITER $
CREATE PROCEDURE getCandidatosByEleccion(id INTEGER)
BEGIN
  SELECT * FROM (
    (SELECT idCandidato, idEstudiante1, idEstudiante2, concat(nombre," ",apellido1," ",apellido2) as nombre1, programa as programa1  
    FROM (SELECT * FROM (SELECT * FROM Candidato WHERE idEleccion = id)al1 INNER JOIN estudiante WHERE identificacion = al1.idEstudiante1)al2 NATURAL JOIN usuario)al3
    NATURAL JOIN 
    (SELECT idCandidato, idEstudiante1, idEstudiante2, concat(nombre," ",apellido1," ",apellido2) as nombre2, programa as programa2  
    FROM (SELECT * FROM (SELECT * FROM Candidato WHERE idEleccion = id)al1 INNER JOIN estudiante WHERE identificacion = al1.idEstudiante2)al2 NATURAL JOIN usuario)al4
  );
END $
DELIMITER ;

-- Define un disparador (trigger) para registrar votos en blanco después de insertar una convocatoria
DELIMITER $
CREATE TRIGGER registraVotoBlanco
AFTER INSERT ON convocatoria
FOR EACH ROW
BEGIN
  -- Inserta una nueva fila en la tabla de candidatos con la opción de voto en blanco
...
-- Cambia la contraseña del usuario 'root' para que utilice el método de autenticación de MySQL nativo
