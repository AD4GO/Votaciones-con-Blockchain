...
export default {
  // Propiedades que recibe el componente como parámetros
  props: ['candidato', 'voted', 'datos'],
  data() {
    return {
      // Estado local para el ID del candidato votado
  ...
  methods: {
    ...
    // Método para realizar la acción de votar por un candidato
    async votar(idCandidato) {
      try {
        await this.$swal.fire({
          // Confirmación del usuario antes de realizar la votación
          ...
          if (result.isConfirmed) {
            await this.$swal.fire({
              // Mensaje de proceso de transacción mientras se vota
              ...
              didOpen: async () => {
                ...
                this.$swal.update({
                  ...
                  text: "Se registró correctamente en la blockchain",
                  icon: "success",
                  showConfirmButton: true
                });
                // Emitir un evento para indicar que se debe recargar la información
                this.$emit("cargar");
                this.$swal.hideLoading();
              }
            });
          } else if (result.isDenied) {
            // Mensaje si el usuario decide no votar
